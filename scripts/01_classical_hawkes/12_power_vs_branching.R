# 12_power_vs_branching.R
#
# Power study:
# How often does the covariate-adjusted bootstrap LR test detect
...
branching_values <- c(0, 0.05, 0.10, 0.20, 0.30, 0.40, 0.50)
...
simulate_covariate_hawkes <- function(
  ...
  while (t < T_end) {
    ...
    X_candidate <- approx(grid, X, xout = t_candidate, rule = 2)$y
...
write.csv(
  results,
  "results/power_vs_branching_replicates.csv",
  row.names = FALSE
)

write.csv(
  summary,
  "results/power_vs_branching_summary.csv",
  row.names = FALSE
)
...
p_power <- ggplot(
  ...
  labs(
    title = "Power of the covariate-adjusted Hawkes test",
    subtitle = paste0(
...
ggsave(
  "figures/power_vs_branching_rejection_rate.pdf",
  p_power,
  width = 7,
  height = 4
)
...
ggsave(
  "figures/power_vs_branching_rejection_rate.png",
  p_power,
  width = 7,
  height = 4,
  dpi = 300
...
p_branching <- ggplot(
  ...
  labs(
    title = "Estimated branching ratio versus true branching",
    x = "true branching ratio",
...
ggsave(
  "figures/power_vs_branching_estimates.pdf",
  p_branching,
  width = 7,
  height = 4
)
...
ggsave(
  "figures/power_vs_branching_estimates.png",
  p_branching,
  width = 7,
  height = 4,
  dpi = 300
...
cat("\nSaved:\n")
cat("- results/power_vs_branching_replicates.csv\n")
cat("- results/power_vs_branching_summary.csv\n")
cat("- figures/power_vs_branching_rejection_rate.pdf/png\n")
cat("- figures/power_vs_branching_estimates.pdf/png\n")